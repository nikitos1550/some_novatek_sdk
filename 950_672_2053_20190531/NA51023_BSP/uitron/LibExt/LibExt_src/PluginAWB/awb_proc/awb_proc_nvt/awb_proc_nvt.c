...
static void AWB_getCA(UINT32 id, UINT16 *R, UINT16 *G, UINT16 *B, UINT16 *IR, UINT16 *Rth, UINT16 *Gth, UINT16 *Bth, UINT16 *Cnt, UINT32 Size)
{
    ...
    if (awb_app_mode == AWB_APPMODE_NORMAL) {
        ...
        for (i = 0; i < AWB_WIN_TOTAL; i++) {
            CA1Info.r[i] = ((CA1Info.r[i]*DGain) >> (19 - AWB_ACC_BIT));
            CA1Info.g[i] = ((CA1Info.g[i]*DGain) >> (19 - AWB_ACC_BIT));
            CA1Info.b[i] = ((CA1Info.b[i]*DGain) >> (19 - AWB_ACC_BIT));
            CA1Info.ir[i] = ((CA1Info.ir[i]*DGain) >> (19 - AWB_ACC_BIT));
            CA1Info.r_th[i] = ((CA1Info.r_th[i]*DGain) >> (19 - AWB_ACC_BIT));
            CA1Info.g_th[i] = ((CA1Info.g_th[i]*DGain) >> (19 - AWB_ACC_BIT));
            CA1Info.b_th[i] = ((CA1Info.b_th[i]*DGain) >> (19 - AWB_ACC_BIT));
