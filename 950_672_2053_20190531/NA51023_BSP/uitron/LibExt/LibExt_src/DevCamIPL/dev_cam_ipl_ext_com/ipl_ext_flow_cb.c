...
void IPL_FCB_ImageFlip(IPL_PROC_ID Id, UINT32 SelIdx, UINT32 Value)
{
	...
	IPL_CtrlSetInfor(FlipInfo.Id, IPLCTRL_FLIP_TYPE, flip_type);
	...
	if (rt == E_OK) {
		...
		if (rt == E_OK) {
			...
			cfa_info.id = Id;
			cfa_info.idx = IPL_CtrlGetHalSetId(Id);
			cfa_info.cfa = IPL_CtrlGetInfor(Id, IPLCTRL_SIE_PRVSTPIX);

			IPL_Ctrl_Runtime_Chg((1 << FlipInfo.Id), IPC_Chg_CFA, (void*)&cfa_info);
			IPL_Ctrl_Runtime_Chg((1 << FlipInfo.Id), IPC_Chg_Load, NULL);
		}
	} else {
		DBG_ERR("Id: %d, Set Sensor Flip fail: rt:%d\r\n", Id, rt);
	}

	FlipInfo.type = IPL_CtrlGetInfor(Id, IPLCTRL_IPL_FLIP_TYPE);
