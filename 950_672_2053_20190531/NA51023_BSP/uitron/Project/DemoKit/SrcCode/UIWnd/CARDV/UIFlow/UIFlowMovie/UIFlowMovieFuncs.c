...
BOOL FlowMovie_IsStorageErr(BOOL IsCheckFull)
{
    ...
    if (TRUE == IsCheckFull)
    {
        ...
        if ((g_MovRecMaxTime <= 3) && (SysGetFlag(FL_MOVIE_CYCLIC_REC) == MOVIE_CYCLICREC_OFF)) {

        if (g_MovRecMaxTime <= MOVIE_REC_TIME_MIN)
        {
            debug_msg("******check storage full\r\n");
...
extern void FlowMovie_Icon_WIFILinkOK_DrawDateTime(void);
...
void FlowMovie_OnTimer1SecIndex(void)
{
    ...
    case MOV_ST_REC | MOV_ST_ZOOM:
		...
		if (UxCtrl_IsShow(&UIMenuWndWiFiModuleLink_YMD_StaticCtrl))
        {
			FlowMovie_Icon_WIFILink_DrawDateTime();//update datetime
        }
		if (UxCtrl_IsShow(&UIMenuWndWiFiModuleLink_YMD_StaticCtrl))
        {
			FlowMovie_Icon_WIFILinkOK_DrawDateTime();
        }
        break;
