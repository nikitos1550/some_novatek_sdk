    ...
    else if ((pSection = PStore_OpenSection(PS_SYS_PARAM, PS_RDWR)) != E_PS_SECHDLER)
    {
        ...
        else
        {
            PStore_CloseSection(pSection);
				...
				if (uiFWUpdate)
			{
               memset(currentInfo.strSSID,0,32);
               memset(currentInfo.strPASSPHRASE,0,32);
				Reset_MenuInfo();
            }
...
void SysCheckFlag(void)
{
	...
	SysLimitFlag(FL_PARKING_MONITONING,   0,  PARKING_MONITONING_MAX,       DEFAULT_PARKING_MONITONING);
	#endif
...
void SysResetFlag(void)
{
    ...
    //#NT#Check SSID and passphrase and reset it if illegal.
    #if (GSENSOR_FUNCTION == ENABLE)
    SysSetFlag(FL_PARKING_MONITONING,            DEFAULT_PARKING_MONITONING);
	#endif
if(currentInfo.strSSID[0] == 0xFF)
    {
        memset(currentInfo.strSSID,'\0', NVT_WSC_MAX_SSID_LEN);
...
void SysExeMenuSettingFuncs(void)
{
    ...
    Ux_SendEvent(&CustomMovieObjCtrl,   NVTEVT_EXE_PARKING_MONITINING,  1,  SysGetFlag(FL_PARKING_MONITONING));
...
#if (_WATCH_DOG_FUNC_ == ENABLE)//2016wdt
extern UINT32 gWDT_AE_Cnt;
BOOL g_bWDT_isWakeUp = FALSE;
void WDT_Init(void)
{
    wdt_open();
    wdt_setConfig(WDT_CONFIG_ID_MODE,WDT_MODE_RESET);
    wdt_setConfig(WDT_CONFIG_ID_TIMEOUT,10000);
    wdt_setConfig(WDT_CONFIG_ID_USERDATA,0x12345678);
    wdt_trigger();
    wdt_enable();
    g_bWDT_isWakeUp = TRUE;
    //debug_msg("^G%s ok\r\n",__func__);
}
void WDT_Wakeup(UINT32 idx)
{
    //wdt_trigger();
    //wdt_enable();
    WDT_Init();
...
void WDT_Sleep(UINT32 idx)
{
    //wdt_trigger();
    //wdt_disable();
    wdt_close();
    g_bWDT_isWakeUp = FALSE;
    //debug_msg("^G%s %d\r\n",__func__,idx);
}
void WDT_Trigger(UINT32 idx)
{
    wdt_trigger();
    //debug_msg("%d\r\n",gWDT_AE_Cnt);
}
BOOL WDT_IsWakeup(void)
{
    return g_bWDT_isWakeUp;
}
void WDT_SensorVDSignal_Check(UINT32 overcnt)
{
	static UINT32 pre_wdt_ae_cnt=0xffffffff,ae_hang_cnt=0;
	int curMode =0;
    curMode =System_GetState(SYS_STATE_CURRMODE);

	 if(g_bWDT_isWakeUp ==0)
    	 return;
...
#define OVERTIME_SHUTDOWN_TIMER_INTERVAL   12*1000 // ms
static volatile UINT32  guiOverTimeShutDownTimerID = TIMER_INVALID;
void WDT_OverTimeShutDownTimerHdl(UINT32 uiEvent)
{
    //debug_msg("^G%s\r\n",__func__);
    timer_close(guiOverTimeShutDownTimerID);
    //DxSys_WDT_Reset();
    GxSystem_SWResetNOW();
}

void WDT_OverTimeShutDownTimer(void)
{
    if (timer_open((UINT *)&guiOverTimeShutDownTimerID, WDT_OverTimeShutDownTimerHdl) != E_OK)
        return;

    //debug_msg("^G%s %d\r\n",__func__,guiOverTimeShutDownTimerID);

    timer_cfg(guiOverTimeShutDownTimerID, OVERTIME_SHUTDOWN_TIMER_INTERVAL*1000, TIMER_MODE_ONE_SHOT | TIMER_MODE_ENABLE_INT, TIMER_STATE_PLAY);
}

void WDT_OverTimeShutDownClose(void)
{
    //debug_msg("^G%s\r\n",__func__);
    timer_close(guiOverTimeShutDownTimerID);
}
//delete file when card full use in timelapse      @van.20170821

UINT8 CardFull_deleteFile(void)
{
    ...
    showFreeSize = (FLOAT)uiFreeSpace/(FLOAT)(1024*1024*1024);

	 FileNum = FileDB_GetTotalFileNum(FileDBHdl);
	 debug_msg("showFreeSize : %f	 FileNum = %d  ++++++++\r\n", showFreeSize,FileNum);

	if(showFreeSize < 1 && UI_GetData(FL_MOVIE_CYCLIC_REC) != MOVIE_CYCLICREC_OFF)
	{
		for(i=0;i<FileNum;i++)
		{
			pFileAttr = FileDB_SearhFile(FileDBHdl, i);
			if(!M_IsReadOnly(pFileAttr->attrib))
			{
				//debug_msg("index = %d  , file :%s attri: %d   FileNum = %d ++++++++++\r\n", i,pFileAttr->filename,pFileAttr->attrib,FileNum);

				sta=FileSys_DeleteFile(pFileAttr->filePath);
				//debug_msg("sts = %d    wait: %d     SD size: %f      +++\r\n", sta, FileSys_WaitFinish(), (FLOAT)(FileSys_GetDiskInfo(FST_INFO_FREE_SPACE))/(FLOAT)(1024*1024*1024));

				if((FLOAT)(FileSys_GetDiskInfo(FST_INFO_FREE_SPACE))/(FLOAT)(1024*1024*1024) >1)
					return 0;
				/*
				if(FileDB_DeleteFile(FileDBHdl,i))
					debug_msg("delete %s sucess!!\r\n", pFileAttr->filePath);
				debug_msg("  ===========   %f    =================\r\n", (FLOAT)(FileSys_GetDiskInfo(FST_INFO_FREE_SPACE))/(FLOAT)(1024*1024*1024));
				if((FLOAT)(FileSys_GetDiskInfo(FST_INFO_FREE_SPACE))/(FLOAT)(1024*1024*1024) >0.8)
					return 0;
					*/
			}
		}
