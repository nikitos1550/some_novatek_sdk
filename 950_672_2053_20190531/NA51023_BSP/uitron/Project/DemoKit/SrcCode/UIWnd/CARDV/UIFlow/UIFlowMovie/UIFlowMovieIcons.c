...
void FlowMovie_Icon_WIFILink_DrawDateTime(void)
{
 RTC_DATE    Date;
 RTC_TIME    Time;
    // display Date/Time string in movie mode
    ...
    UxStatic_SetData(&UIMenuWndWiFiModuleLink_YMD_StaticCtrl,STATIC_VALUE,Txt_Pointer(date_str));
    UxStatic_SetData(&UIMenuWndWiFiModuleLink_HMS_StaticCtrl,STATIC_VALUE,Txt_Pointer(time_str));
    UxCtrl_SetShow(&UIMenuWndWiFiModuleLink_YMD_StaticCtrl,TRUE);
    UxCtrl_SetShow(&UIMenuWndWiFiModuleLink_HMS_StaticCtrl,TRUE);
}

void FlowMovie_Icon_WIFILinkOK_DrawDateTime(void)
{
 RTC_DATE    Date;
 RTC_TIME    Time;
    ...
    sprintf(date_str,"%04d/%02d/%02d",Date.s.year,Date.s.month,Date.s.day);
    sprintf(time_str,"%02d:%02d:%02d",Time.s.hour,Time.s.minute,Time.s.second);
    UxStatic_SetData(&UIMenuWndWiFiModuleLinkOK_YMD_StaticCtrl,STATIC_VALUE,Txt_Pointer(date_str));
    UxStatic_SetData(&UIMenuWndWiFiModuleLinkOK_HMS_StaticCtrl,STATIC_VALUE,Txt_Pointer(time_str));
    UxCtrl_SetShow(&UIMenuWndWiFiModuleLinkOK_YMD_StaticCtrl,TRUE);
    UxCtrl_SetShow(&UIMenuWndWiFiModuleLinkOK_HMS_StaticCtrl,TRUE);
}
void FlowMovie_IconDrawLock(VControl *pCtrl, BOOL bDraw)
{
	if(bDraw == FALSE)
	{
...
void FlowMovieWifi_IconDrawRecTime(VControl *pCtrl)
{
   if (WiFiCmd_GetStatus()==2)
   {
        ...
        UxStatic_SetData(pCtrl,STATIC_VALUE,Txt_Pointer(g_RecCurrTimeStr));
        UxCtrl_SetShow(pCtrl, TRUE);
	//debug_err(("====FlowMovieWifi_IconDrawRecTime============\r\n"));
    }
}
...
void FlowMovie_Icon_WIFILink_DrawPIM(BOOL bDraw)
{
    UxCtrl_SetShow(&UIMenuWndWiFiModuleLink_StaticIcon_PIMCCtrl, bDraw);
}
