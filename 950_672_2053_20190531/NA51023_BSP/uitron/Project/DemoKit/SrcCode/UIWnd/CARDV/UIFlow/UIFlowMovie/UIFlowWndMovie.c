...
INT32 UIFlowWndMovie_OnWIFI_ONOFF(VControl *, UINT32, UINT32 *);
...
EVENT_ITEM(NVTEVT_KEY_ENTER,UIFlowWndMovie_OnKeySelect )
//EVENT_ITEM(NVTEVT_KEY_DOWN, UIFlowWndMovie_OnWIFI_ONOFF)
EVENT_ITEM(NVTEVT_KEY_ZOOMIN, UIFlowWndMovie_OnWIFI_ONOFF)
...
void Rec_led_OnOff(void);
void WIFI_Rec_led_OnOff(void);
...
//control for REC LED
BOOL    Rec_Led_FLAG=0;
BOOL   WIFI_Rec_Led_FLAG=0;
BOOL First_Vedio_Rec  = FALSE;  //Don't play the recoding vedio in Movie Mode  by FAN
...
//#NT#2015/08/05#KS Hung -end
		...
		case MOV_ST_REC|MOV_ST_ZOOM:
			...
			//if (FlowMovie_GetRecCurrTime() >= 1)
			{
				...
				BKG_PostEvent(NVTEVT_BKW_STOPREC_PROCESS);				LED_RED_TurnOn();
#else
				//bBK_StopRec =FALSE;
...
//MUST open before UIFlowWndMovie_Initparam, because Initparam call GSensor_SetSensitivity.
	...
	// Enable Motion Detect function in starting up movie mode
 	//关闭移动侦测，避免因此功能为停车监控不关机和部分客户误打开这个功能，移动侦测仅一次有效    van 2016.05.20
		if(IsFirstOpen == TRUE)
		{
		...
		//设置RO文件夹占用比例   van 2016.05.24
		SMediaRec_SetROPercent(30);
		//debug_msg("get RO percent : %d          ++++++\r\n", MediaRec_GetROPercent());
			IsFirstOpen = FALSE;
		}
	if (SysGetFlag(FL_MOVIE_MOTION_DET) == MOVIE_MOTIONDET_ON) {
		g_uiRecordIngMotionDet = TRUE;
...
//#if (GSENSOR_FUNCTION == ENABLE)
	//GSensor_close();//20181222- gsensor close
//#endif
...
INT32 UIFlowWndMovie_OnWIFI_ONOFF(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	UINT32  uiKeyAct;
	...
	#if _ADAS_FUNC_
    if (ADAS_GetAppsStatus())//close ADAS before WIFI ON,
    {
    	debug_msg("====================== stop ADAS!!!!!	===========================\r\n");
        ADAS_AppsStop();
		//ADAS_SaveFlag = TRUE;
	...
	if(System_GetState(SYS_STATE_CARD) != CARD_REMOVED)
	{
       //  Ux_PostEvent(NVTEVT_KEY_ENTER, 1, NVTEVT_KEY_PRESS);
         //Delay_DelayMs(500 );        // fast record
        UINT32 paramArray[1] = {NVTEVT_KEY_PRESS};
        UIFlowWndMovie_OnExeRecord(pCtrl, 1, paramArray);
	}
               case MOV_ST_REC:
			case MOV_ST_REC|MOV_ST_ZOOM:
	  	debug_msg("KEY DEL PRESS!!!!!\r\n");
