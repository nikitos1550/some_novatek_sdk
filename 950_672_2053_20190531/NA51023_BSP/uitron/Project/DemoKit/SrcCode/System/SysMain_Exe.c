...
BOOL PWRON_SRC_PWR_KEY = FALSE;
BOOL PWRON_SRC_PWR_GSENSOR = FALSE;
...
INT32 System_OnBoot(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	if (paramNum == 1 && paramArray[0] == 1) { //power-on end
		...
		if (g_bBootFromALM == FALSE) {
			...
			if (System_GetState(SYS_STATE_POWERON) == SYSTEM_POWERON_SAFE) {
				// add power up sound
#if(UI_FUNC==ENABLE)
				UISound_Play(DEMOSOUND_SOUND_OPEN_START_TONE);

				//UISound_Play(DEMOSOUND_SOUND_MACHINE_STOPPAGE_TONE);
				//UISound_Play(DEMOSOUND_SOUND_TIME_TONE);
#endif
			}
#endif
			//#NT#2016/03/07#KCHong -begin
    ...
    #if 1//z
        uiStatus = FileSys_GetAttrib("A:\\FW96672A.bin" ,&ucAttrib);
        if(uiStatus == FST_STA_OK)
        {
            if(ucAttrib & FS_ATTRIB_READ)
            {
                debug_err(("=====The update file is Only Read!!! ======\r\n"));
            }else{
                FileSys_DeleteFile("A:\\FW96672A.bin");
                debug_err(("===== Delete the update file!!! ======\r\n"));
            }
        }

     #endif
    }
       // debug_err(("===== %d ======\r\n",GxSystem_GetPowerOnSource()));
    if(GxSystem_GetPowerOnSource() == GX_PWRON_SRC_PWR_SW){
       PWRON_SRC_PWR_KEY = TRUE;
    }else if(GxSystem_GetPowerOnSource() == GX_PWRON_SRC_PWR_VBAT){
       PWRON_SRC_PWR_GSENSOR= TRUE;
    }
...
INT32 System_OnShutdown(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	if (paramNum == 1 && paramArray[0] == 0) { //power-off begin
		...
		if(WDT_IsWakeup()==TRUE)
			WDT_Sleep(WDT_POWER_SHUTDOWN);
		WDT_OverTimeShutDownTimer();
#endif
...
#endif
#if (GSENSOR_FUNCTION == ENABLE) //add by van

		if(SysGetFlag(FL_PARKING_MONITONING) != PARKING_MONITONING_OFF)
		{
			GSensor_ParkingMode();
			debug_msg("===================  gsensor Parking mode open!!  ===================\r\n");
		}
		else
		{
			debug_msg("===================  closing Normal  ===================\r\n");
		}
#endif
		...
		if (g_bBootFromALM == FALSE) {
			...
			UISound_Play(DEMOSOUND_SOUND_GOODBYE_TONE);
