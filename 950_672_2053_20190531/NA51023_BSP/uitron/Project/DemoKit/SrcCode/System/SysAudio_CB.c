...
extern void SPK_TurnOff(void);
extern void SPK_TurnON(void);
...
Sound_start=FALSE;
	...
	DBG_IND("end\r\n");
    if((Sound_start==TRUE||FlowMovie_GetMovDataState()==1||((System_GetState( SYS_STATE_CURRMODE)==PRIMARY_MODE_PLAYBACK)))&& gMovData.State !=  MOV_ST_MENU)
    {Delay_DelayMs( 1);
        SPK_TurnON();
    }
    else
