...
static INT32 PipView_OnDraw_2sensor(APPDISP_VIEW_DRAW *pDraw) //PIP = Picture In Picture
{
	...
	if (pDraw->viewcnt == 2)
	{
		...
		{
			if(pDraw->p_dst_img != pDraw->p_src_img[0])
		...
		{
			if(pDraw->p_dst_img != pDraw->p_src_img[1])
			        GxImg_FillData(pDraw->p_dst_img, NULL, COLOR_YUV_BLACK) ;
		...
		else if(UI_GetData(FL_DUAL_CAM) == DUALCAM_BOTH2)
		{
			...
			dst_region.h = pDraw->p_dst_img->Height / 2;
			if(pDraw->p_src_img[0]){
				GxImg_ScaleData(pDraw->p_src_img[0], REGION_MATCH_IMG, pDraw->p_src_img[1], &dst_region);
			}
		...
		else// if(UI_GetData(FL_DUAL_CAM) == DUALCAM_BOTH)
		{
			if(pDraw->p_dst_img != pDraw->p_src_img[0])
				GxImg_FillData(pDraw->p_dst_img, NULL, COLOR_YUV_BLACK) ;
