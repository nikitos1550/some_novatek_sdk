...
extern BOOL    WIFI_Rec_Led_FLAG;
extern  void WIFI_Rec_led_OnOff(void);
...
INT32 WiFiCmd_OnExeSetMovieParkingSensitivity(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	UINT32 Data;
	...
	if (WiFiCmd_GetStatus() != WIFI_MOV_ST_RECORD) {
		Ux_SendEvent(&CustomMovieObjCtrl, NVTEVT_EXE_PARKING_MONITINING, 1, Data);
	}
...
INT32 WiFiCmd_OnMovieOneSec(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	if (WiFiCmd_GetStatus() == WIFI_MOV_ST_RECORD && paramNum) {
    	...
    	WIFI_Rec_Led_FLAG=!WIFI_Rec_Led_FLAG;
    	WIFI_Rec_led_OnOff();
    }
...
INT32 UIFlowWndWiFiMovie_OnExeSetParkingMotion(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	UINT32 Data;
