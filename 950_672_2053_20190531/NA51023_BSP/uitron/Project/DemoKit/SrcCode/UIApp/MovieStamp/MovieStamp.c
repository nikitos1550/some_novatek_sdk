		...
		if ((g_uiMovieStampSetup[i] & STAMP_SWITCH_MASK) == STAMP_ON) {
			...
			DBG_DUMP("time2: %d us\r\n", Perf_GetCurrent() - uiTime);
                   ...
                   if (g_bWaterLogoEnable[i]){
	                   ...
	                   MovieStamp_VsUpdateOsd(i, TRUE, (16+i), 2, g_MovieWaterInfo[i].uiXPos, g_MovieWaterInfo[i].uiYPos, g_MovieWaterInfo[i].uiWidth, g_MovieWaterInfo[i].uiHeight, (void*)g_MovieWaterInfo[i].uiWaterLogoYAddr);
                   }
#endif
...
void MovieStamp_RestoreData(UINT32 vid)
{
	...
	if (g_bWaterLogoEnable[vid]){
		MovieStamp_VsUpdateOsd(vid, TRUE, (VS_WATERLOGO_ID+vid), VS_WATERLOGO_REGION, g_MovieWaterInfo[vid].uiXPos, g_MovieWaterInfo[vid].uiYPos, g_MovieWaterInfo[vid].uiWidth, g_MovieWaterInfo[vid].uiHeight, (void*)g_MovieWaterInfo[vid].uiWaterLogoYAddr);
	}
#endif
...
#if defined (WATERLOGO_FUNCTION) && (WATERLOGO_FUNCTION == ENABLE)
		if (g_bWaterLogoEnable[vid]){
		       MovieStamp_VsUpdateOsd(vid, FALSE, (16+vid), 2, g_MovieWaterInfo[vid].uiXPos, g_MovieWaterInfo[vid].uiYPos, g_MovieWaterInfo[vid].uiWidth, g_MovieWaterInfo[vid].uiHeight, (void*)g_MovieWaterInfo[vid].uiWaterLogoYAddr);
		}
#endif
...
void MovieStamp_DrawMultiStamp(UINT32 uiVidEncId, UINT32 CntId, UPOINT *Pos, char *pStr)
{
	...
	MovieStamp_VsUpdateOsd(uiVidEncId, TRUE, (VS_MULTISTAMP_ID+CntId), (VS_MULTISTAMP_REGION+CntId), Pos->x, Pos->y, VsFontOut[CntId].GenImg.Width, VsFontOut[CntId].GenImg.Height, (void*)pVsMultiFontDataAddr[CntId]);
#endif
}
#endif
