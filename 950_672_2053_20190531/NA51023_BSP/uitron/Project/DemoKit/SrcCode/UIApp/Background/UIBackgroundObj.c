...
//----------------------------------------------
    ...
    FileSys_WaitFinish();
    GxStrg_SetConfig(FILE_CFG_SUPPORT_EXFAT, FALSE);
    GxStrg_OpenDevice(0,pStrgDXH);
    FileSys_WaitFinish();
	//#NT#2016/05/30#Lincy Lin -end
	ret = FileSys_FormatDisk(pStrgDXH, FALSE);
	DBG_DUMP("FormatDisk ret=%d\r\n", ret);
    //#NT#2018/9/14#Philex Lin - begin
   //----------------------------------------
    GxStrg_CloseDevice(0);
    FileSys_WaitFinish();
    GxStrg_SetConfig(FILE_CFG_SUPPORT_EXFAT, FALSE);
    GxStrg_OpenDevice(0,pStrgDXH);
    FileSys_WaitFinish();
    //#NT#2018/9/14#Philex Lin - end

//-----------------------------------------------
    GxStrg_CloseDevice(0);
    FileSys_WaitFinish();
    GxStrg_SetConfig(FILE_CFG_SUPPORT_EXFAT, TRUE);
    GxStrg_OpenDevice(0,pStrgDXH);
    FileSys_WaitFinish();
	if (ret == FST_STA_OK) {
        UI_SetData(FL_FSStatus,FS_INIT_OK);//-----------------------------20190109LIU
		// reset file ID (for FileDB)
#if USE_FILEDB
		if (UI_GetData(FL_IsUseFileDB)) {
			Ux_SendEvent(&UISetupObjCtrl, NVTEVT_FILEID_RESET, 0);
