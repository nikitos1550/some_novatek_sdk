...
static void sen_power_ctrl_ar0330p(SENSOR_ID id, POW_CTRL_FLAG flag)
{
	...
	case POW_CTRL_TURN_ON:
		...
		// set power on sequence
		gpio_setDir(S_GPIO_13, GPIO_DIR_OUTPUT);
		gpio_clearPin(S_GPIO_13);
		gpio_setDir(S_GPIO_14, GPIO_DIR_OUTPUT);
		gpio_clearPin(S_GPIO_14);
		
		Delay_DelayMs(10);
		gpio_setPin(S_GPIO_13);
		Delay_DelayMs(10);
		gpio_setPin(S_GPIO_14);
...
/*
    ...
    init_obj.signal_info.hd_phase=SENSOR_PHASE_FALLING;
    init_obj.signal_info.vd_phase=SENSOR_PHASE_FALLING;
    init_obj.signal_info.data_phase=SENSOR_PHASE_FALLING;*/
...
#endif
#if (defined(_CMOS_TP2825_))
	{"CMOS_TP2825",  sensor_get_drv_tab_tp2825,  sensor_get_obj_tp2825,  sen_power_ctrl_tp2825,  sen_detplug_ctrl_tp2825, NULL},
#endif
